...
namespace Demo.Actors.Ask
{
    public static class FutureDemo
    {
        public static void Start()
        {
            ...
            // Get Result
            Console.ReadKey();
            var result = actorA.Ask("[Russ]-Am I there?", TimeSpan.FromSeconds(1)).Result;
            actorC.Tell($"FirstAsk: {result}");

            // WhenAll asks complete continue.
            var actorATask = actorA.Ask("[Doug]-Is Anderson there? ", TimeSpan.FromSeconds(1));
            var actorAaTask = actorA.Ask("[Jay]-Is Anderson there? ", TimeSpan.FromSeconds(1));
            var actorBTask = actorB.Ask("[Marcus]-What about Neo? ", TimeSpan.FromSeconds(5));
